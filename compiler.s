...
%define MAKE_LITERAL_SYMBOL(str) MAKE_LITERAL T_SYMBOL, dq str       
...
%macro EXTEND_ENV 1
	...
	%%COPY_MINOR_VECS:
		cmp rbx, %1
		je %%FINISH_COPY_MINOR_VECS
		mov rdx, [rbp + (WORD_SIZE * 2)]
		shl rbx, 3
		add rdx, rbx
		mov rdx, [rdx]
		...
		inc rcx
		jmp %%COPY_MINOR_VECS
	%%FINISH_COPY_MINOR_VECS:
	
	mov rcx, 0
	%%ADD_PARAMS:		
		cmp rcx, [rbp + (WORD_SIZE * 3)]
		je %%FINISH_ADD_PARAMS
		mov rbx, [rax]
		shl rcx, 3
		add rbx, rcx
		shr rcx, 3
...
%macro ADJUST_STACK 1
mov rcx, [rsp + (WORD_SIZE * 2)]
	sub rcx, %1
	
	cmp rcx, 0
	jl %%ADJUST_BELOW_ARGS
	...
	%%ADJUST_ABOVE_ARGS:
		...
		%%FINISH_LIST:
			mov [rsp + WORD_SIZE * (2 + %1)], rdx
			sub [rsp + (WORD_SIZE * 2)], rcx
			mov rbx, %1
			add rbx, 2			
			cmp rcx, 0
			je %%FINISH_ADJUST
			...
			%%LOOP_SHIFT:
				cmp rbx, 0
				jl %%FINISH_SHIFT
				lea rsi, [rsp + (WORD_SIZE * rbx)]
				lea rdi, [rsi + (WORD_SIZE * rcx)]
				mov rsi, [rsi]
				mov [rdi], rsi
				dec rbx
				jmp %%LOOP_SHIFT
	...
	%%ADJUST_BELOW_ARGS:
		...
		mov rbx, [rsp + (WORD_SIZE * 3)]
		add rbx, 3
		mov rsi, 0

			%%SHIFT_DOWN:
				cmp rsi, rbx
				jge %%FINISH_SHIFT_DOWN
				lea rdi, [rsp + WORD_SIZE * (rsi + 1)]
				mov rdx, rdi
				sub rdx, WORD_SIZE
				mov rdi, [rdi]
				mov [rdx], rdi
			...
			%%FINISH_SHIFT_DOWN:
				...
				mov qword[rsp + WORD_SIZE * (2 + %1)], SOB_NIL_ADDRESS
	%%FINISH_ADJUST:
%endmacro

%macro FIX_STACK_TP 1
	mov r9, [rbp] 
	...
	%%STACK_OVERWRITE:
		...
		mov rsi, [rsi]
		dec rbx
		lea rdi, [rbp + (WORD_SIZE * rbx)]
		mov [rdi], rsi
		inc rcx
		jmp %%STACK_OVERWRITE
		
	%%ADJUST_STACKPOINTER:
		lea rsp, [rbp + (WORD_SIZE * rbx)]
		mov rbp, r9			
%endmacro
